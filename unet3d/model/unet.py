...
def get_unet(input_img, n_filters, batchnorm=True):
    ...
    # expansive path
    u6 = Conv3DTranspose(n_filters*8, kernel_size=3, strides=2, batchnorm=batchnorm)(c5)
    u6 = concatenate([u6, c4])
    c6 = conv3d_block(u6, n_filters*8, kernel_size=3, batchnorm=batchnorm)

    u7 = Conv3DTranspose(n_filters*4, kernel_size=3, strides=2, batchnorm=batchnorm)(c6)
    u7 = concatenate([u7, c3])
    c7 = conv3d_block(u7, n_filters*4, kernel_size=3, batchnorm=batchnorm)

    u8 = Conv3DTranspose(n_filters*2, kernel_size=3, strides=2, batchnorm=batchnorm)(c7)
    u8 = concatenate([u8, c2])
    c8 = conv3d_block(u8, n_filters*2, kernel_size=3, batchnorm=batchnorm)

    u9 = Conv3DTranspose(n_filters*1, kernel_size=3, strides=2, batchnorm=batchnorm)(c8)
