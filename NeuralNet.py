...
def data_generator(inputs_path, labels_path, start, stop, how_many=1):
    ...
    input_images = np.zeros(shape=(how_many, 4, 240, 240, 155))  # how_many - wielkość paczki danych
    masks = np.zeros(shape=(how_many, 240, 240, 155))
    for brain_nr in range(start, stop):
        for seq_nr in range(4):
            ...
            img_data /= max_value1
            input_images[brain_nr % how_many][seq_nr] = img_data
        ...
        # generator podaje tablicę o wielkości (how_many, 4, 240, 240, 155)
        # (how_many mózgów w 4 sekwencjach)
...
# brains_number = 285
# dane uczące: 80% próbek, dane testowe: reszta
...
test_generator = data_generator(inputs_path, labels_path, train_set_size, brains_number)
